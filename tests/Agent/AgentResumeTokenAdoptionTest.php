...
/**
 * ADR 0005: the resume token is stamped on the suspended ToolCallMessage in chat
 * history, and a payload-carrying chat()/stream()/structured() call with no explicit
 * resumeToken adopts it from the history tail.
 */
class AgentResumeTokenAdoptionTest extends TestCase
{
    ...
    public function testSuspendStampsResumeTokenOnHistoryTail(): void
    {
        ...
        $this->assertSame($agent->getWorkflowId(), $tail->getResumeToken());
    }

    public function testResumeAdoptsTokenFromHistory(): void
    {
        $history = new InMemoryChatHistory();
        $executor = new WorkflowExecutor(new LocalStepEngine(new InMemoryPersistence()));
        $searchTool = new SearchTool();
        ...
        // A new execution cycle: NO resumeToken — only the shared chat history.
        $agent2 = Agent::make();
        ...
        $this->assertSame($agent1->getWorkflowId(), $agent2->getWorkflowId());
    ...
    public function testHistoryTokenOverridesExplicitResumeToken(): void
    {
        $history = new InMemoryChatHistory();
        $executor = new WorkflowExecutor(new LocalStepEngine(new InMemoryPersistence()));
        $searchTool = new SearchTool();
        ...
        // an explicit (here: wrong) resumeToken passed to make().
        $agent2 = Agent::make(resumeToken: 'some_other_workflow');
        $agent2->setChatHistory($history);
        $agent2->setAiProvider($provider);
        $agent2->addTool($searchTool);
        $agent2->addMiddleware(ToolNode::class, new ToolApproval([SearchTool::class]));
    ...
    public function testKeepsOwnWorkflowIdWhenHistoryHasNoToken(): void
    {
        // A payload against a thread with no stamped tail (empty history here) leaves
        // the workflowId untouched — the fallback that keeps explicit-token resumes
        // of non-approval suspensions working.
        $agent = Agent::make(resumeToken: 'my_explicit_token');
        ...
        $this->assertSame('my_explicit_token', $agent->getWorkflowId());
    }

    public function testSameInstanceResumeKeepsOwnWorkflowId(): void
    {
        $history = new InMemoryChatHistory();
        $executor = new WorkflowExecutor(new LocalStepEngine(new InMemoryPersistence()));
        $searchTool = new SearchTool();
        ...
        $this->assertSame($workflowId, $agent->getWorkflowId());
        $this->assertSame('Here are the search results...', $message->getContent());
    }
}
