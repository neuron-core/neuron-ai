...
class FakeAIProviderTest extends TestCase
{
    ...
    public function test_stream_yields_text_chunks(): void
    {
        ...
        $generator = $provider->stream([new UserMessage('Hi')], function (ToolCallMessage $toolCall) {
            yield $toolCall;
            yield new ToolCallResultMessage($toolCall->getTools());
        });
    ...
    public function test_stream_records_request(): void
    {
        ...
        $provider->stream([new UserMessage('Hi')], function (ToolCallMessage $toolCall) {
            yield $toolCall;
            yield new ToolCallResultMessage($toolCall->getTools());
        });
    ...
    public function test_assert_method_call_count(): void
    {
        ...
        $provider->chat([new UserMessage('a')]);
        $provider->chat([new UserMessage('b')]);
        $provider->stream([new UserMessage('c')], function (ToolCallMessage $toolCall) {
            yield $toolCall;
            yield new ToolCallResultMessage($toolCall->getTools());
        });
