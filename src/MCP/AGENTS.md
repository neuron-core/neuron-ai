...
- `McpConnector` is the developer-facing entry point. `tools()` lists the server's tools, applies the `only()` / `exclude()` filters, and wraps each one in an `McpTool`: a `Tool` subclass whose `__invoke()` forwards the call back through the connector. Its input schema is translated by the shared `ToolPropertyFactory` into regular `ToolProperty` / `ArrayProperty` / `ObjectProperty` definitions, recursively including nested objects and array items. The factory rejects references and composition schemas that the property model cannot represent; see `src/Tools/AGENTS.md`.
