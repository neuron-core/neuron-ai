...
- `submitInputs($payload, $translator, idempotencyKey: $key)` returns an `ExecutionRequest` with the translated answer and the observed run/attempt fences. Call `$workflow->run($request)` or `$workflow->events($request)`. Native Agent approval/tool-result helpers return the same request type.
