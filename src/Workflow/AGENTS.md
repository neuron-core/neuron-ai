...
- `submitInputs($payload, $translator, idempotencyKey: $key)` returns a `PendingExecution` holding the workflow and an immutable `ExecutionRequest` with the translated answer and the observed address/run/attempt fences. Chain `->run()` or `->events()` on the result. Native Agent approval/tool-result helpers return the same pending execution type. Submission and creating a stream do not execute nodes or write persistence.
