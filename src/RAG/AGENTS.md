...
- `PreProcessNode` reads the question from the start event, initializes `state->request` from it and the segment's instructions (the role `AgentStartNode` plays in the Agent) and runs the pre-processors (query rewriting, expansion). Nothing is written to chat history before inference: pending messages commit only after the provider call succeeds, so a failed turn never leaves a dangling user message.
