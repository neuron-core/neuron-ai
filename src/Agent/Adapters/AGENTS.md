...
The protocol boundary between Neuron's native streamed objects and UI protocols such as AG-UI and the Vercel AI SDK data stream. `NativeAdapter` is the same boundary for a destination that speaks no UI protocol.
